...
class Loss(nn.Module):
    def __init__(self, focal, no_back_in_output):
        super(Loss, self).__init__()
        if focal:
            self.loss = DiceFocalLoss(gamma=2.0, softmax=True, to_onehot_y=True, batch=True)
            if no_back_in_output:
                self.loss = DiceFocalLoss(gamma=2.0, sigmoid=True, to_onehot_y=False, batch=True)
            
        else:
            self.loss = DiceCELoss(softmax=True, to_onehot_y=True, batch=True)
            if no_back_in_output:
                self.loss = DiceCELoss(sigmoid=True, to_onehot_y=False, batch=True)
