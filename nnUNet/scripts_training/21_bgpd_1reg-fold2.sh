...
export CUDA_VISIBLE_DEVICES=0 && python3 ../main.py --no_back_in_output  --data /data/private_data/${task}_3d --results /results/bgpd_results/$name/fold-2 --deep_supervision --depth 6 --filters 64 96 128 192 256 384 512 --min_fmap 2 --scheduler --learning_rate 0.0003 --epochs 70 --nfolds 3 --fold 2 --amp --gpus 1 --task $task --patience 20 --save_ckpt

echo Save predicts $name fold-2!

export CUDA_VISIBLE_DEVICES=0 && python ../main.py --no_back_in_output  --exec_mode predict --task ${task}_3d --data /data/private_data/${task}_3d --dim 3 --fold 2 --nfolds 3 --ckpt_path /results/bgpd_results/$name/fold-2/checkpoints/best*.ckpt --results /results/bgpd_infer/$name --amp --tta --save_preds
