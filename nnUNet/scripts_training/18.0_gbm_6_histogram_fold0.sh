...
# export CUDA_VISIBLE_DEVICES=1 && python3 ../main.py --brats --data /data/private_data/${task}_3d --results /results/gbm_results/$name --deep_supervision --depth 6 --filters 64 96 128 192 256 384 512 --min_fmap 2 --scheduler --learning_rate 0.0003 --epochs 100 --nfolds 3 --fold 0 --amp --gpus 1 --task $task --save_ckpt
...
python metrics.py --path_to_pred /results/gbm_infer/$name/*fold=0_tta --path_to_target /data_anvar/public_datasets/preproc_study/gbm/6_hist/$name/labels --out /results/gbm_infer/$name/metrics_gbm_${name}_fold-0.csv
