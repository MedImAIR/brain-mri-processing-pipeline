...
class DataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        print(f"{len(self.train_imgs)} training, {len(self.val_imgs)} validation, {len(self.test_imgs)} test examples")
...
def get_test_fnames(args, data_path, meta=None):
    ...
    if args.exec_mode == "predict" and args.train_inf:
        train_idx, val_idx = list(kfold.split(test_imgs))[args.fold]
        test_imgs = sorted(get_split(test_imgs, val_idx))
        if meta is not None:
            meta = sorted(get_split(meta, val_idx))
