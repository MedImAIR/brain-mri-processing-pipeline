...
parser.add_argument('--path_to_brain_masks',  default= '/anvar/public_datasets/preproc_study/gbm/1_reg_ss_shared/', help='path to mask native space images', type=str)
...
def calculate_metrics(path_to_pred, path_to_target, spaces = [1,1,1], out = '/home/polina/glioma/all_dice_metrics.csv'  ):
    ...
    for ids in tqdm(os.listdir(pred_folder)):
        sub = ids[:-4]
        targets = ants.image_read(str(target_folder) + '/' +  f'{sub}_seg.nii.gz')
...
if __name__ == "__main__":
    ...
    nohup python /home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/check_native_space_metrics.py --path_to_pred /anvar/public_datasets/preproc_study/gbm/inference/7a_resample_aug_300/ --path_to_current_space /anvar/public_datasets/preproc_study/gbm/7a_resamp/ --namefile 4a_resamp_aug > 4a_resamp_aug.out &
    
    nohup python /home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/check_native_space_metrics.py --path_to_pred /anvar/public_datasets/preproc_study/gbm/inference/2a_interp/ --path_to_current_space /anvar/public_datasets/preproc_study/gbm/2a_interp/ --namefile 2a_interp > home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/2a_interp.out &
    ...
    nohup python /home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/check_native_space_metrics.py --path_to_pred /anvar/public_datasets/preproc_study/gbm/inference/3a_susan/ --path_to_current_space /anvar/public_datasets/preproc_study/gbm/3a_atlas/ --namefile 3d_susan > /home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/3d_susan.out &
    
     nohup python /home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/check_native_space_metrics.py --path_to_pred /anvar/public_datasets/preproc_study/gbm/inference/6_hist/ --path_to_current_space /anvar/public_datasets/preproc_study/gbm/3a_atlas/ --namefile 6_hist > 6_hist.out &
     
     nohup python /home/kate/projects/brain-mri-processing-pipeline/main_pipeline/stats/check_native_space_metrics.py --path_to_pred /anvar/public_datasets/preproc_study/gbm/inference/gbm_3a_atlas/ --path_to_current_space /anvar/public_datasets/preproc_study/gbm/3a_atlas/ --namefile 5_ss_mask_native > 5_ss_mask_native.out &
    ...
    path_brain_masks = args.path_to_brain_masks
            ...
            if 'fold={}'.format(fold) in fold_path:
                ...
                for i in os.listdir(old_orig):
                    if os.path.isdir(old_orig + i):
                        if not os.path.isfile(cur_folder + '/' + i + '.npz'):
                            print(old_orig + i)
                            try:
                                # *.npz archives sometimes can ve recognised wrong
                                data = np.load(glob(
                                    infer_path + infer_dir + '/' + i + '*.npz')[0])
                                ...
                                orient = new_orig_ct1.get_orientation()
#                                 img_mask = ants.image_read(path_brain_masks + i + '/' + 'CT1_mask.nii.gz').reorient_image2(orient)
...
#                                 new_array[0] = new_img_0.numpy()*img_mask.numpy()
#                                 new_array[1] = new_img_1.numpy()*img_mask.numpy()
#                                 new_array[2] = new_img_2.numpy()*img_mask.numpy()
                                new_array[0] = new_img_0.numpy()
                                new_array[1] = new_img_1.numpy()
                                new_array[2] = new_img_2.numpy()
                                
                                np.savez_compressed(cur_folder + '/' + i + '.npz',new_array.transpose(0,3,2,1).astype('float16'))
