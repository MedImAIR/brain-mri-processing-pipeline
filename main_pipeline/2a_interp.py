...
parser.add_argument('--output', type=str, default='/anvar/public_datasets/preproc_study/gbm/2a_interp/', 
                    help= 'output folder')
...
if __name__ == "__main__":
    ...
    """Pipeline with CT1 Rigid registration and interpolation to template, n4 and Z-score calculation
       nohup python 2a_interp.py > 2a_interp.out &
    """
    ...
    for subject in subjects:
        ...
        # Reorient fixed
        img_fixed = ants.reorient_image2(img_fixed, orientation = 'LAI')
        mask_fixed = ants.reorient_image2(mask_fixed, orientation = 'LAI')
...
#         img_fixed_res = ants.resample_image_to_target(img_fixed, img_target, interp_type='linear')
        ...
        for name in args.movingfilenames:
            ...
            img_moving = ants.reorient_image2(img_moving, orientation = 'LAI')
            # Image registration
